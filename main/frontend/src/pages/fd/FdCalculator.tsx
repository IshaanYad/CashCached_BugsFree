...
export function FdCalculator() {
  const { t } = useI18n();
  const { formatTokens, formatConvertedTokens, preferredCurrency } =
  ...
  useEffect(() => {
    ...
    if (
      ...
      if (
        ...
        ns.push(
          `${t("calculator.validation.amount")}: ${formatTokens(
            selectedProduct.minAmount
          )} ${t("common.to")} ${formatTokens(selectedProduct.maxAmount)}`
        );
      }
    }
    if (freqVal !== "yearly") {
                ...
                >
                  <div>
                    <p className="font-medium">
                      {formatTokens(100000)} · {parseInt(rate.tenure)}{" "}
                ...
                <div className="flex justify-between">
                  ...
                  <div className="flex flex-col items-end">
                    <span className="text-3xl font-bold text-primary">
                      {formatTokens(result.maturityAmount)}
                    </span>
                ...
                <div className="flex justify-between">
                  ...
                  <div className="flex flex-col items-end">
                    <span className="text-lg font-semibold">
                      {formatTokens(result.interestEarned)}
                    </span>
                    <span className="text-xs text-muted-foreground">
                      {formatConvertedTokens(
